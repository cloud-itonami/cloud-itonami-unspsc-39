(ns elecinstall.operation
  "ElecInstallOperationActor -- one install-and-diagnostics
  coordination request = one supervised actor run, expressed as a
  langgraph-clj StateGraph. The advisor (DiagnosticsAdvisor) is sealed
  into a single node (:advise); its proposal is ALWAYS routed through
  the Electrical Install Governor (:govern) and the rollout phase gate
  ...
  each operation is auditable and checkpointed. An install operator's
  lifecycle is advanced by MANY ops (diagnostic-reading logging /
  repair scheduling / commissioning coordination), each independent.

  Human-in-the-loop = real approval workflow:
  `interrupt-before #{:request-approval}` pauses the actor and hands the
  decision to a human qualified electrician/coordinator. The approver
  resumes with `{:approval {:status :approved}}` (or :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [elecinstall.advisor :as advisor]
            [elecinstall.governor :as governor]
...
(defn build
  "Compiles an ElecInstallOperationActor graph bound to `store`.
  opts:
    :advisor      -- an `elecinstall.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; DiagnosticsAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Electrical Install Governor -- independent censor.
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        ...
                        :reason (or reason
                                    (cond (:high-stakes? verdict) :requires-electrician-approval
